...
This file documents the rules and workflows that change Codex's behavior in this repository. Reference material lives in linked files — read those when relevant, not by default.
...
**NotebookLM markdown sync (auto):** A `PostToolUse` hook in `.Codex/settings.json` runs `scripts/sync_instructor_md.sh` after any `Edit`/`Write`/`NotebookEdit` whose path ends in `_instructor.ipynb`, regenerating `_notebook_lm/<basename>.md` for NotebookLM podcast ingestion. The directory is gitignored. For direct Jupyter edits (outside Codex), run `bash scripts/sync_instructor_md.sh` manually, or watch with `fswatch -o notebooks/*_instructor.ipynb | xargs -n1 -I{} bash scripts/sync_instructor_md.sh`.
...
- **Commit messages:** `<type>: <subject>` where type is `feat`, `fix`, `docs`, `chore`, `build`, `refactor`. Always include `Co-Authored-By: Codex Sonnet 4.5 <noreply@anthropic.com>`.
